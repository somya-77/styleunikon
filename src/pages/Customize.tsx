...
import { SIZES, COLORS, formatPrice } from '@/data/products';
...
const Customize = () => {
  ...
  const [logoPosition] = useState({ x: 50, y: 40 });
  ...
  const shirtBg = selectedColor === 'BLACK' ? '#1A1A1A'
    ...
    : selectedColor === 'SAND' ? '#D2B48C' : '#F5F5F5';
      ...
      <div className="px-8 lg:px-16 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
          ...
          <div className="border-2 border-foreground aspect-[3/4] relative overflow-hidden flex items-center justify-center" style={{ backgroundColor: shirtBg }}>
            ...
            {customText && (
              <div className="absolute z-10 select-none pointer-events-none text-center px-4" style={{
                top: `${logoPosition.y}%`, left: `${logoPosition.x}%`, transform: 'translate(-50%, -50%)',
                fontFamily: selectedFont, color: textColor, fontSize: 'clamp(16px, 3vw, 32px)', fontWeight: 'bold',
              }}>{customText}</div>
            )}
            {logoFile && (
              <img src={logoFile} alt="Custom logo" className="absolute z-10 max-w-[40%] max-h-[30%] object-contain"
                style={{ top: `${logoPosition.y + 15}%`, left: `${logoPosition.x}%`, transform: 'translate(-50%, -50%)' }} />
            )}
            {!customText && !logoFile && (
